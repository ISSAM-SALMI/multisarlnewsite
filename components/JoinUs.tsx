...
export const JoinUs: React.FC = () => {
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-navy-900 pt-20 pb-10 md:pt-24 md:pb-12 transition-colors duration-300">
        ...
        {!jobId ? (
          ...
          <div className="max-w-4xl mx-auto">
            <div className="text-center mb-8 md:mb-12">
              <h1 className="text-2xl md:text-4xl font-black text-navy-900 dark:text-white mb-3 md:mb-4">
            ...
            <div className="bg-white dark:bg-navy-800 rounded-xl p-4 shadow-sm border border-slate-100 dark:border-navy-700 mb-6 md:mb-8">
              <div className="flex flex-col md:flex-row gap-3 md:gap-4">
                <div className="relative flex-grow">
                  ...
                  <input 
                    ...
                    className="w-full pl-10 pr-4 py-2.5 md:py-3 rounded-lg bg-slate-50 dark:bg-navy-900 border border-slate-200 dark:border-navy-600 text-navy-900 dark:text-white focus:ring-2 focus:ring-electric-red focus:border-transparent outline-none transition-all text-sm md:text-base"
                    value={searchTerm}
              ...
              {filteredJobs.length > 0 ? (
                filteredJobs.map((jobItem) => (
                  <div 
                    ...
                    className="bg-white dark:bg-navy-800 rounded-xl p-4 md:p-6 shadow-lg hover:shadow-xl transition-all cursor-pointer border border-transparent hover:border-electric-red/30 group relative overflow-hidden active:scale-[0.98]"
                  >
                    <div className="flex flex-row items-start gap-3 md:gap-4">
                      <div className="p-2.5 md:p-3 rounded-lg bg-slate-100 dark:bg-navy-700 text-electric-red group-hover:bg-electric-red group-hover:text-white transition-colors shrink-0">
                        <jobItem.icon className="w-5 h-5 md:w-6 md:h-6" />
                      </div>
                      <div className="flex-grow min-w-0">
                        <div className="flex flex-wrap items-center gap-2 mb-1">
                          <h3 className="text-base md:text-xl font-bold text-navy-900 dark:text-white group-hover:text-electric-red transition-colors leading-tight mt-1">
                            {jobItem.title}
                          </h3>
                        </div>
                        <div className="flex flex-wrap gap-2 mt-2">
                          <span className="inline-block px-2.5 py-0.5 text-[10px] md:text-xs font-semibold rounded-full bg-slate-100 dark:bg-navy-900 text-slate-600 dark:text-slate-300">
                            {jobItem.type}
                          </span>
                          <span className={`inline-block px-2.5 py-0.5 text-[10px] md:text-xs font-semibold rounded-full border ${getStatusColor(jobItem.status)}`}>
                            {jobItem.status}
                          </span>
                        </div>
                      </div>
          ...
          <div className="max-w-6xl mx-auto">
            <button 
              ...
              className="flex items-center gap-2 text-slate-600 dark:text-slate-400 hover:text-electric-red mb-6 md:mb-8 transition-colors font-medium text-sm md:text-base"
            >
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 md:gap-8">
              ...
              <div className="lg:col-span-2 space-y-4 md:space-y-6">
                {/* Header Card */}
                <div className="bg-white dark:bg-navy-800 rounded-2xl p-6 md:p-8 shadow-sm border border-slate-100 dark:border-navy-700">
                  <div className="flex flex-col md:flex-row items-start gap-4 md:gap-6">
                    <div className="p-3 md:p-4 rounded-xl bg-slate-50 dark:bg-navy-900 text-electric-red border border-slate-100 dark:border-navy-700 shrink-0">
                      {job && <job.icon className="w-8 h-8 md:w-10 md:h-10" />}
                    </div>
                    <div>
                      <h1 className="text-xl md:text-3xl font-bold text-navy-900 dark:text-white mb-3 md:mb-4 leading-tight">
                        {job?.title}
                      </h1>
                      <div className="flex flex-wrap gap-2 md:gap-3">
                        <span className="inline-flex items-center gap-1.5 px-2.5 py-1 md:px-3 md:py-1.5 text-xs md:text-sm font-medium rounded-lg bg-slate-100 dark:bg-navy-900 text-slate-600 dark:text-slate-300">
                        ...
                        {job && (
                          <span className={`inline-flex items-center gap-1.5 px-2.5 py-1 md:px-3 md:py-1.5 text-xs md:text-sm font-medium rounded-lg border ${getStatusColor(job.status)}`}>
                            <div className={`w-1.5 h-1.5 md:w-2 md:h-2 rounded-full ${job.status === 'Ouvert' ? 'bg-green-500' : 'bg-red-500'}`}></div>
                            {job.status}
                          </span>
                        )}
                      </div>
                ...
                <div className="bg-white dark:bg-navy-800 rounded-2xl p-6 md:p-8 shadow-sm border border-slate-100 dark:border-navy-700">
                  <h2 className="text-lg md:text-xl font-bold text-navy-900 dark:text-white mb-4 md:mb-6 flex items-center gap-3">
                    <div className="w-1 h-5 md:h-6 bg-electric-red rounded-full"></div>
                    Missions & Responsabilités
                  </h2>
                  <ul className="space-y-3 md:space-y-4">
                    {job?.missions.map((mission, idx) => (
                      <li key={idx} className="flex items-start gap-3 md:gap-4 group">
                        <div className="mt-1 w-5 h-5 md:w-6 md:h-6 rounded-full bg-electric-red/10 flex items-center justify-center flex-shrink-0 group-hover:bg-electric-red/20 transition-colors">
                          <CheckCircle2 className="w-3.5 h-3.5 md:w-4 md:h-4 text-electric-red" />
                        </div>
                        <span className="text-sm md:text-base text-slate-600 dark:text-slate-300 leading-relaxed">{mission}</span>
                      </li>
                    ))}
                  </ul>
                </div>
                ...
                <div className="bg-white dark:bg-navy-800 rounded-2xl p-6 md:p-8 shadow-sm border border-slate-100 dark:border-navy-700">
                  ...
                  <ul className="space-y-3 md:space-y-4">
                    {job?.profile.map((item, idx) => (
                      <li key={idx} className="flex items-start gap-3 md:gap-4 group">
                        <div className="mt-1.5 w-5 h-5 md:w-6 md:h-6 rounded-full bg-blue-50 dark:bg-blue-900/20 flex items-center justify-center flex-shrink-0 group-hover:bg-blue-100 dark:group-hover:bg-blue-900/30 transition-colors">
                          <div className="w-1.5 h-1.5 md:w-2 md:h-2 rounded-full bg-blue-500"></div>
                        </div>
                        <span className="text-sm md:text-base text-slate-600 dark:text-slate-300 leading-relaxed">{item}</span>
                      </li>
                    ))}
                  </ul>
                </div>
                ...
                <div className="sticky top-24 space-y-6">
                  ...
                  <div className="bg-white dark:bg-navy-900 rounded-2xl p-6 shadow-xl text-navy-900 dark:text-white relative overflow-hidden border border-slate-100 dark:border-navy-700">
                    ...
                    <div className="relative z-10">
                      <h3 className="text-lg md:text-xl font-bold mb-2">Intéressé(e) ?</h3>
                      ...
                      {job?.status !== 'Clôturé' ? (
                        <>
                          <p className="text-xs md:text-sm text-slate-500 dark:text-gray-400 mb-2">Postulez à cette adresse :</p>
                          <div className="bg-slate-50 dark:bg-white/5 rounded-xl p-4 border border-slate-200 dark:border-white/10 text-center">
                            <a 
                              ...
                              className="text-base md:text-lg font-bold text-electric-red hover:text-red-600 dark:hover:text-red-400 transition-colors flex items-center justify-center gap-2 break-all"
                            >
